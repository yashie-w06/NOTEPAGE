(() => {
  ...
  const blockStyle = document.querySelector('#blockStyle');
  const STORAGE_KEY = 'dos-note-autosave-v1';
  let savedRange = null, selectedImage = null, saveTimer, styleSyncFrame;
  ...
  function currentBlock(){
    ...
    return element.closest('h1,h2,h3,p,pre,blockquote,li,div');
  }
  function currentBlockStyle(){
    const block=currentBlock();
    if(!block)return null;
    const styled=block.matches('h1,h2,h3,p,pre,blockquote')?block:block.closest('h1,h2,h3,p,pre,blockquote');
    return styled?.tagName.toLowerCase()||'p';
  }
  function syncBlockStyle(){
    ...
    styleSyncFrame=requestAnimationFrame(()=>{
      ...
      if(style&&blockStyle.value!==style)blockStyle.value=style;
    });
  }
  ...
  function formatBlock(tag){
    ...
    if(block?.tagName==='LI'){
      const nested=selectionElement()?.closest('h1,h2,h3,p,pre,blockquote');
      if(nested&&block.contains(nested)&&nested.tagName.toLowerCase()!==tag)replaceBlock(nested,tag);
    }else if(block && editor.contains(block) && block.tagName.toLowerCase()!==tag){
      if(block.matches('div,p,h1,h2,h3,pre,blockquote'))block=replaceBlock(block,tag);
      else if(before?.matches('div,p,h1,h2,h3,pre,blockquote'))block=replaceBlock(before,tag);
    }
    blockStyle.value=tag;editor.focus();rememberCaret();syncBlockStyle();changed();
  }
  function stripHeadingContainers(list){
    list.querySelectorAll('h1,h2,h3').forEach(heading=>{
      const fragment=document.createDocumentFragment();
      while(heading.firstChild)fragment.append(heading.firstChild);
      heading.replaceWith(fragment);
    });
  }
  function toggleList(commandName){
    restoreCaret();
    const wasInList=Boolean(selectionElement()?.closest('ul,ol'));
    if(currentBlockStyle()!=='p')document.execCommand('formatBlock',false,'<p>');
    document.execCommand(commandName,false,null);
    const list=selectionElement()?.closest('ul,ol');
    if(list)stripHeadingContainers(list);
    if(wasInList&&!list)document.execCommand('formatBlock',false,'<p>');
    blockStyle.value='p';editor.focus();rememberCaret();syncBlockStyle();changed();
  }
  ...
  function leaveHeading(heading){
    const selection=getSelection();
    if(!selection.rangeCount)return;
    const caret=selection.getRangeAt(0);
    const tail=document.createRange();tail.selectNodeContents(heading);tail.setStart(caret.startContainer,caret.startOffset);
    const trailing=tail.extractContents();
    const paragraph=document.createElement('p');paragraph.append(trailing);
    ...
    blockStyle.value='p';editor.focus();changed();
  }
  function safePastedHtml(html){
    const doc=new DOMParser().parseFromString(html,'text/html');
    doc.querySelectorAll('script,style,link,meta,base,iframe,object,embed,form,input,button,textarea,select').forEach(node=>node.remove());
  ...
  function insertHtml(html){
    restoreCaret();
    const clean=safePastedHtml(html);
    if(!document.execCommand('insertHTML',false,clean)){
      const selection=getSelection(),range=selection.getRangeAt(0),fragment=range.createContextualFragment(clean);
    ...
    rememberCaret();syncBlockStyle();changed();
  }
  function exportHtml(){
    updateToc();
  ...
  document.querySelectorAll('[data-cmd]').forEach(b=>b.onclick=()=>{
    if(b.dataset.cmd==='insertUnorderedList'||b.dataset.cmd==='insertOrderedList')toggleList(b.dataset.cmd);
    else command(b.dataset.cmd);
  });
  blockStyle.onchange=e=>formatBlock(e.target.value);
  document.querySelector('.toolbar').addEventListener('pointerdown',rememberCaret);
  ...
  editor.addEventListener('keydown',e=>{
    ...
    if(heading&&editor.contains(heading)){e.preventDefault();leaveHeading(heading);return;}
    const block=node?.closest?.('pre,blockquote');if(!block||!editor.contains(block))return;
    const after=document.createRange();after.selectNodeContents(block);after.setStart(selection.anchorNode,selection.anchorOffset);
    if(after.toString().trim()||(!block.innerText.endsWith('\n')&&block.innerText.trim()))return;
    e.preventDefault();leaveStyledBlock(block,selection.anchorNode);
  });
  editor.addEventListener('keyup',()=>{rememberCaret();syncBlockStyle();});
  editor.addEventListener('mouseup',()=>{rememberCaret();syncBlockStyle();});
  editor.addEventListener('focus',syncBlockStyle);editor.addEventListener('input',()=>{syncBlockStyle();changed();});title.addEventListener('input',changed);
  editor.addEventListener('paste',e=>{
    const html=e.clipboardData.getData('text/html');
    if(html){e.preventDefault();insertHtml(html);return;}
  ...
  document.addEventListener('selectionchange',()=>{if(editor.contains(getSelection().anchorNode)){rememberCaret();syncBlockStyle();}});
  document.addEventListener('keydown',e=>{
    if(!(e.ctrlKey||e.metaKey)||e.altKey)return;
    const key=e.key.toLowerCase();
    if(key==='s'){e.preventDefault();exportHtml();return;}
    if(key==='o'){e.preventDefault();document.querySelector('#fileInput').click();return;}
    const styles={'1':'h1','2':'h2','3':'h3','4':'p'};
    if(styles[key]&&editor.contains(getSelection().anchorNode)){e.preventDefault();formatBlock(styles[key]);}
  });
